...
//
// Material values survive weather grading; no colour-keyed khaki remap.
...
const FRAG = /* glsl */ `
  ...
  void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
    ...
    c *= 1.02; // gain: reclaim what the composer chain takes
    c.r *= 1.0 + uTemp * 0.085;
    c.b *= 1.0 - uTemp * 0.085;
    c.g *= 1.0 + uTintG * 0.05;
    ...
    c += uLift * pow(1.0 - clamp(l, 0.0, 1.0), 1.7);
...
export const WEATHER_GRADE: Record<WeatherType, GradeParams> = {
  clear:    { temp: 0.38,  tintG: 0.05,  sat: 1.14, contrast: 1.1,  lift: 0.008 },
  overcast: { temp: 0.16,  tintG: 0.05,  sat: 1.02, contrast: 1.03, lift: 0.018 },
  rain:     { temp: 0.22,  tintG: 0.06,  sat: 0.96, contrast: 1.04, lift: 0.008 },
  mud:      { temp: 0.22,  tintG: 0.08,  sat: 1.0,  contrast: 1.01, lift: 0.012 },
  snow:     { temp: -0.12, tintG: 0.0,   sat: 0.92, contrast: 1.05, lift: 0.022 },
};

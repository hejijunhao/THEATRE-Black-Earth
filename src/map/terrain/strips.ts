...
const FIELD_COLORS = ['#e0c878', '#c4a056', '#a07838', '#d8bc70', '#8a7040', '#6e4c28'].map(rgb);
const SHELTER = rgb('#5a4a28');
const DIRT = rgb('#7a5830');
const FURROW = rgb('#5c3e20');
...
export function stripFrame(wx: number, wz: number): StripFrame {
  ...
  const stripW = 0.22 + ihash(rx, rz, 103) * 0.20;
...
export function fieldColor(wx: number, wz: number): RGB {
  ...
  if (furrow > 0.72) c = mix(c, FURROW, 0.28 * (furrow - 0.72) / 0.28);

  // Shelter-belt / headland darkening on both axes.
  if (f.edgeU < 0.10) c = mix(c, SHELTER, 0.42 * (1 - f.edgeU / 0.10));
  if (f.edgeV < 0.08) c = mix(c, DIRT, 0.34 * (1 - f.edgeV / 0.08));
...
export function parcelRelief(wx: number, wz: number): number {
  ...
  const rx = Math.floor(wx / 14);
  const rz = Math.floor(wz / 14);
  const terrace = (ihash(rx * 517 + f.strip, rz, 113) - 0.5) * 0.034;
  const lip = Math.max(0, 0.07 - f.edgeU) * 0.22 + Math.max(0, 0.06 - f.edgeV) * 0.16;
  const wave = Math.sin((f.u / f.stripW) * Math.PI * 2) * 0.007;

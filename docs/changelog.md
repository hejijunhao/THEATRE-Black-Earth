...
- **Punches the cost-faded interior wash** so a darker umber soil stain leads (near fill outranks the seam). Adjacent discs overlap and the origin hex is stained, so the wash is one territorial blob instead of a honeycomb of inset plates.
...
Presentation only. Hierarchy from 0.2.12–0.2.17 stays locked: LOD hex caps, select ground ring, Kupiansk–Sloviansk boot, instrument strip + sector rail. No new panels. No HUD rearrange. Select → move → attack → AAR is unchanged.

0.2.17 passed north khaki and armor silhouettes. The rest frame was still one quiet khaki field, and INF/MECH/ARTY collapsed to ghost plates whenever `tbe-counters` was left on. This cut attacks that stack:

- **Contact-scale dirt.** Strip parcels keep value-split colour, shelter-belt / headland edges, furrow dirt, and a matching heightfield lip. North lift stays far-north so the scar is not washed back into a slab.
- **Non-armor boot read.** Infantry is a dark rank plus a command truck. Mech and artillery stay on hero hulls. All machine shots clear or assert `tbe-counters` off before judging.

The campaign, map, AI, supply and combat math are unchanged. Still not Vic 3 / HoI4 / Civ — contact dirt is a painted + displaced field, not a diorama tile.

## [0.2.17] — 2026-09-17 · Rain veil root-cause + boot machines

Presentation only. Hierarchy from 0.2.12–0.2.16 stays locked: LOD hex caps, select ground ring, Kupiansk–Sloviansk boot, instrument strip + sector rail. No new panels. No HUD rearrange. Select → move → attack → AAR is unchanged.

0.2.16 lifted albedo and fog. The veil and the plates-only mid-zoom survived that. This cut attacks the stack that was still live:
...
Presentation only. Hierarchy from 0.2.12–0.2.14 stays locked: one instrument strip, one sector rail, bench on select. No new panels. No sim deepening. Select → move → attack → AAR is unchanged.

- **LOD is a hard cap.** Counters, standards and select chrome stay inside a hex at every zoom. Select is a ground annulus, not a parchment card — selecting must not blot neighbouring soil.
- **Boot camera is the scar.** Opening frame is the Kupiansk–Sloviansk contact at mid-zoom, not the whole Donbas.
- **North is the same soil.** Fog, hemisphere bounce and fill are warm earth-haze so rain and mud do not drop the far side into a dark-grey veil.

...
| `panzerHero` (inspector-grade panzer: ~19k tri, per-link tracks, bolted wheels, unlit field-green stamp) | `src/assets/panzerHero.ts` + `heroParts.ts` | runtime | same class silhouette at showcase density; covers the Armored class for both factions. 0.2.29: INF-path MeshBasic + authored dark hull / light turret / dark gun so mid-zoom reads as a stamp, not a weathered wash. Geometry unchanged | ✓ no insignia, stamp not NATO plate | 2026-09-20 |
| `mechHero` (inspector-grade tracked IFV: front sprocket, per-link tracks, autocannon turret, rear ramp) | `src/assets/heroMech.ts` + `heroAssemblies.ts` | runtime | Marder-family *class* silhouette. 0.2.20: unlit field-green stamp (dark hull / light top) so turret + autocannon read under rain; geometry from 0.2.19 unchanged | ✓ no insignia, stamp not NATO plate | 2026-09-17 |
| `artilleryHero` (inspector-grade towed 155: split trails, spades, elevated tube, muzzle brake, handwheels) | `src/assets/heroArtillery.ts` | runtime | FH70-family *class* silhouette, firing pose. 0.2.20: same unlit stamp so tube / muzzle brake / trails own the read; geometry from 0.2.19 unchanged | ✓ no insignia, stamp not NATO plate | 2026-09-17 |
...
| `heroFleet` (unit-type → hero geometry registry) | `src/assets/heroFleet.ts` | runtime | one geometry per (type, faction); the map instances it per element. 0.2.29: armor/MECH/ARTY take the unlit stamp; recon keeps the weathered wash | n/a | 2026-09-20 |
| `stampHero` (unlit field-green armor/MECH/ARTY material) | `src/assets/heroParts.ts` (`getStampHeroMaterial`) | runtime | INF-style Basic stamp: dark hull / light turret, sat above the rain veil gate. Armor joined in 0.2.29. Not used by INF or recon | ✓ stamp not NATO plate | 2026-09-20 |
...
| `Vegetation` (map ground cover: steppe tuft clumps, cropland stubble rows, wetland tufts + reeds; blade geometry from `terrainHero`) | `src/map/Vegetation.tsx` | runtime | placement driven by the geodata crop/wetland fractions; cropland rows follow the shared strip frame so albedo, relief and stubble agree | ✓ muted palette on the terrain albedo, comes off the paper map with the other 3D clutter | 2026-09-17 |
| `ifv` (tracked IFV + autocannon) | `src/assets/vehicles.ts` | superseded | archetypal IFV/APC class | ✓ | 2026-08-07 |
| `lightTruck` (soft-skin, canvas bed) | `src/assets/vehicles.ts` | runtime | utility truck class; artillery limber and replacement column. 0.2.19: dark canvas, chassis, windshield so it is not a pale plate | ✓ | 2026-09-17 |
| `commandTruck` (soft-skin + whip antenna) | `src/assets/vehicles.ts` | runtime | infantry command wagon; same scale as `lightTruck`, antenna / bumper / glass so the rank has a machine, not a barn | ✓ | 2026-09-17 |
...
| `makeMiniatureBuild` (composition + state mapping; splits hero vehicles from vertex-coloured props) | `src/assets/units.ts` | runtime | §6.2 state table; echelon layout sized to the base plate. 0.2.29: armor/mech/arty keep hero hulls on the unlit stamp; infantry rank unchanged | ✓ | 2026-09-20 |

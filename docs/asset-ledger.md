...
| `heroFleet` (unit-type → hero geometry registry) | `src/assets/heroFleet.ts` | runtime | one geometry per (type, faction); the map instances it per element. 0.2.20: MECH/ARTY take the unlit stamp, armor/recon keep the weathered wash | n/a | 2026-09-17 |
| `stampHero` (unlit field-green MECH/ARTY material) | `src/assets/heroParts.ts` (`getStampHeroMaterial`) | runtime | INF-style Basic stamp: dark hull / light top, sat above the rain veil gate. Not used by INF or armor | ✓ stamp not NATO plate | 2026-09-17 |
...
| `makeMiniatureBuild` (composition + state mapping; splits hero vehicles from vertex-coloured props) | `src/assets/units.ts` | runtime | §6.2 state table; echelon layout sized to the base plate. 0.2.20: infantry rank unchanged; mech/arty keep hero hulls but leave the shared wash for the unlit stamp | ✓ | 2026-09-17 |

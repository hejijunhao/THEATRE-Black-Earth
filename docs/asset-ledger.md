...
| `mechHero` (inspector-grade tracked IFV: front sprocket, per-link tracks, autocannon turret, rear ramp) | `src/assets/heroMech.ts` + `heroAssemblies.ts` | runtime | Marder-family *class* silhouette. 0.2.19: thicker track band, BODY roof, oversized autocannon so boot mid-zoom reads hull/turret/gun, not a pale plate | ✓ no insignia, muted paint | 2026-09-17 |
| `artilleryHero` (inspector-grade towed 155: split trails, spades, elevated tube, muzzle brake, handwheels) | `src/assets/heroArtillery.ts` | runtime | FH70-family *class* silhouette, firing pose. 0.2.19: thicker elevated tube, heavier trails and wheels so the gun, not the carriage plate, owns the boot read | ✓ no insignia, muted paint | 2026-09-17 |

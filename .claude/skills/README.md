...
> **Provenance.** Vendored from [`MengTo/Skills`](https://github.com/MengTo/Skills)
> `agent-skills/game-development/`, commit `46abf78` (2026-07-28), MIT-licensed —
> see [`LICENSE.upstream`](LICENSE.upstream). Copied verbatim apart from this
> note. To refresh: re-clone upstream and re-copy that directory over this one.
>
> **Fit with THEATRE: BLACK EARTH.** These skills assume a real-time
> Three.js action game (frame budgets, hitboxes, movesets, loot). This project
> is a *turn-based* pure-function simulation with a read-only R3F view layer —
> so treat their gameplay-architecture advice as suggestive, not normative, and
> keep [`CLAUDE.md`](../../CLAUDE.md)'s invariants (no `Math.random()` in
> `src/game/`, JSON-serializable `GameState`, rules never in `store.ts`)
> authoritative wherever the two disagree. The render/QA/asset skills
> (`optimize-threejs-games`, `test-playable-web-games`, `create-game-vfx`,
> `build-hybrid-game-assets`, `build-game-camera-controls`) transfer most
> cleanly. `build-vesperfall-review-assets` is specific to another project's
> asset catalog and is kept only for completeness.
